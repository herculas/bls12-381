...
TEST(TestUtil, BytesToUint64BE) {
    ...
    uint64_t result = bls12_381::util::bit_operation::be_bytes_to_uint64(array);
    EXPECT_EQ(result, expected);
}

TEST(TestUtil, Uint64BEToBytes) {
    uint64_t value = 0x0001020304050607;
    std::array<uint8_t, sizeof(uint64_t)> array = bls12_381::util::bit_operation::uint64_to_be_bytes(value);
...
TEST(TestUtil, BytesToUint64LE) {
    ...
    uint64_t result = bls12_381::util::bit_operation::le_bytes_to_uint64(array);
    EXPECT_EQ(result, expected);
}

TEST(TestUtil, Uint64LEToBytes) {
    uint64_t value = 0x0001020304050607;
    std::array<uint8_t, sizeof(uint64_t)> array = bls12_381::util::bit_operation::uint64_to_le_bytes(value);
