...
TEST(TestFp2, Equality) {
    bls12_381::field::Fp2 a{
            bls12_381::field::Fp({1, 2, 3, 4, 5, 6}),
            bls12_381::field::Fp({7, 8, 9, 10, 11, 12})
    };
    bls12_381::field::Fp2 b{
            bls12_381::field::Fp({1, 2, 3, 4, 5, 6}),
            bls12_381::field::Fp({7, 8, 9, 10, 11, 12})
    };
    EXPECT_TRUE(a == b);
...
TEST(TestFp2, Sqrt) {
    ...
    EXPECT_EQ(a.sqrt().value().square(), a);
    ...
    EXPECT_EQ(b.sqrt().value().square(), b);
    ...
    EXPECT_EQ(c.sqrt().value().square(), c);

    bls12_381::field::Fp2 d{
            bls12_381::field::Fp({
...
TEST(TestFp2, Inv) {
    ...
    EXPECT_FALSE(bls12_381::field::Fp2::zero().invert().has_value());
}

TEST(TestFp2, Lexical) {
    EXPECT_FALSE(bls12_381::field::Fp2::zero().lexicographically_largest());
    EXPECT_FALSE(bls12_381::field::Fp2::one().lexicographically_largest());
