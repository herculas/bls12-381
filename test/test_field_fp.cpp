...
TEST(TestFp, FromBytes) {
    std::array<uint8_t, bls12_381::field::Fp::WIDTH * sizeof(uint64_t)> array1 = {
    ...
    std::array<uint8_t, bls12_381::field::Fp::WIDTH * sizeof(uint64_t)> array2 = {
    ...
    bls12_381::field::Fp a = -bls12_381::field::Fp::one();
    bls12_381::field::Fp b = bls12_381::field::Fp::from_bytes(array1).value();
    EXPECT_EQ(a, b);

    std::optional<bls12_381::field::Fp> c = bls12_381::field::Fp::from_bytes(array2);
...
TEST(TestFp, ToBytes) {
    ...
    std::array<uint8_t, bls12_381::field::Fp::WIDTH * sizeof(uint64_t)> bytes{};
    bls12_381::field::Fp b;

    for (int i = 0; i < 100; ++i) {
        a = a.square();
        bytes = a.to_bytes();
        b = bls12_381::field::Fp::from_bytes(bytes).value();
        EXPECT_EQ(a, b);
    }
}
...
TEST(TestFp, Lexical) {
    ...
    EXPECT_FALSE(bls12_381::field::Fp::zero().lexicographically_largest());
    EXPECT_FALSE(bls12_381::field::Fp::one().lexicographically_largest());
