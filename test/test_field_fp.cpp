...
TEST(TestFp, FromBytes) {
    std::array<uint8_t, Fp::WIDTH * sizeof(uint64_t)> const array1 = {
    ...
    std::array<uint8_t, Fp::WIDTH * sizeof(uint64_t)> const array2 = {
    ...
    Fp const a = -Fp::one();
    Fp const b = Fp::from_bytes(array1).value(); // NOLINT(bugprone-unchecked-optional-access)
    EXPECT_EQ(a, b);

    std::optional<Fp> const c = Fp::from_bytes(array2);
...
TEST(TestFp, Sqrt) {
    ...
    // sqrt(4) = -2.
    EXPECT_EQ(a.sqrt().value(), -b); // NOLINT(bugprone-unchecked-optional-access)
}
